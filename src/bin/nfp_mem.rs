...
use rust_nfp_tools::libs::exp_bars::{init_device_bars, ExpansionBar};
...
// Initialize the global CRC-32 checksum
static mut CRC_GLOBAL: u32 = 0xFFFFFFFF;
...
// Function to update the global CRC checksum
pub fn crc32_update(value: u32) {
    let byte = value.to_le_bytes();

    unsafe {
        for &b in &byte {
            CRC_GLOBAL = (CRC_GLOBAL >> 8) ^ CRC32_TABLE[(CRC_GLOBAL ^ b as u32) as usize & 0xFF];
        }
    }
}
...
fn main() {
    ...
    let mut exp_bar = ExpansionBar::new(&cli.pci_bdf);
