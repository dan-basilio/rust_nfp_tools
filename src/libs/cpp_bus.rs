...
impl<'a> CppBus<'a> {
    ...
    fn configure_exp_bar(
        ...
        match self.exp_bar.exp_bar_map {
            MapType::Fixed => {
                // Configure the BAR with fixed mapping.
                self.exp_bar.expansion_bar_fixed_cfg(
                    island.id(),
                    target.id(),
                    action,
                    token,
                    ...
                    cpp_len.id(),
                    self.exp_bar.exp_bar_size,
                );
            }
            MapType::Bulk => {
                // Configure the BAR with bulk mapping.
                self.exp_bar.expansion_bar_bulk_cfg(
                    island.id(),
                    target.id(),
                    token,
                    self.exp_bar.exp_bar_base_addr,
