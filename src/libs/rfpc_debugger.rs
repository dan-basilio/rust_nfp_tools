...
pub fn rfpc_dbg_halt(exp_bar: &mut ExpansionBar, rfpc: &Rfpc) {
    ...
    loop {
        ...
        if dmstatus[0] & RISCV_DBG_DMSTATUS_ALLHALTED != 0 {
            break;
        }
...
pub fn rfpc_dbg_resume(exp_bar: &mut ExpansionBar, rfpc: &Rfpc) {
    ...
    loop {
        ...
        if dmstatus[0] & RISCV_DBG_DMSTATUS_ALLRUNNING != 0 {
            break;
        }
        thread::sleep(Duration::from_millis(100));
    }
...
pub fn rfpc_dbg_read_reg(exp_bar: &mut ExpansionBar, rfpc: &Rfpc, reg_addr: u64) -> u64 {
    ...
    let err_code = rfpc_dbg_abstractcmd(exp_bar, rfpc, 0, command);
    if err_code != 0 {
...
pub fn rfpc_dbg_read_memory(
    ...
    for word_idx in 0..word_len {
        ...
        let err_code = rfpc_dbg_abstractcmd(exp_bar, rfpc, 0, 0x37100a);
        if err_code != 0 {
            panic!("RFPC abstract command returned error {}.", err_code);
        }
        // Read memory word from RFPC GPR a0.
        mem_words.push(rfpc_dbg_read_reg(exp_bar, rfpc, 0x100a));
    }
...
pub fn rfpc_dbg_write_memory(
    ...
    if address != align_addr {
        ...
        let prepend_data =
            rfpc_dbg_read_memory(exp_bar, rfpc, align_addr - prepend_len, prepend_len);
    ...
    for (word_idx, data_word) in mem_words.iter().enumerate() {
        ...
        if rfpc_dbg_abstractcmd(exp_bar, rfpc, 0, 0x33100b) != 0 {
        ...
        if rfpc_dbg_abstractcmd(exp_bar, rfpc, 0, 0x37100a) != 0 {
            panic!("RFPC abstract command returned error.");
        }
    }
