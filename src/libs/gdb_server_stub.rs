...
use crate::libs::mem_access::{mem_write, MuMemoryEngine, MemoryType};
use crate::libs::exp_bars::ExpansionBar;
use crate::libs::rfpc::{Rfpc, RfpcGpr, RfpcCsr, RfpcReg};
...
impl<'a> RspServer<'a> {
    ...
    fn read_gprs(&mut self) -> String {
        ...
        // let mut reg_val = rfpc_dbg_read_reg(self.exp_bar, &rfpc, reg_addr);
        // gprs.push_str(&format!("{:016x}", reg_val.swap_bytes()));
        gprs.push_str(&format!("{:016x}", 0x0000000000000000));

        // reg_addr = RfpcGpr::X1.reg_addr();
        // reg_val = rfpc_dbg_read_reg(self.exp_bar, &rfpc, reg_addr);
        // gprs.push_str(&format!("{:016x}", reg_val.swap_bytes()));
        gprs.push_str(&format!("{:016x}", 0x0000000000000000));
    ...
    fn write_csr(&mut self, packet: Vec<u8>) -> String {
        ...
        if address == 32 {
            ...
            rfpc_dbg_write_reg(self.exp_bar, &rfpc, dpc, value.swap_bytes());
            //rfpc_dbg_resume(self.exp_bar, &rfpc);
        }
    ...
    fn load_segment(&mut self, packet: Vec<u8>) -> String {
        ...
        let mut program_data: Vec<u32> = cast_slice(&packet_data).to_vec();
        // Swap bytes of each element in the vector
        program_data.iter_mut().for_each(|x| *x = x.swap_bytes());
    ...
    fn handle_packet(&mut self, packet: Vec<u8>) -> Option<String> {
        // Extract the command
        let colon_index = packet.iter().position(|&b| b == b':').unwrap_or_else(|| packet.len());
    ...
    pub fn run(&mut self, running: Arc<AtomicBool>) {
        // Make all memory on the chip accessible from the debug group.
        xpb_write(
            self.exp_bar,
            &CppIsland::Rfpc0,
            0x280000,
